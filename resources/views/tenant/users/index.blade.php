...
@section('content')
<!-- Replaced root x-data with a wrapper to ensure AJAX updates don't break Alpine if we swap content -->
<div id="users-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
    
    <!-- Page Header & Stats -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div class="space-y-1">
            <h1 class="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Identity Management</h1>
            <p class="text-muted-foreground text-sm">Manage user access, roles, and platform permissions.</p>
        </div>
        ...
        <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
            <a href="/users" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === null && request('trashed') === null ? 'bg-background text-foreground shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-foreground hover:bg-background/50' }}">
                All Users
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="/users?status=active" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'active' ? 'bg-background text-emerald-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-emerald-600 hover:bg-background/50' }}">
                Active
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="/users?status=inactive" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'inactive' ? 'bg-background text-amber-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-amber-600 hover:bg-background/50' }}">
                Inactive
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="/users?trashed=only" class="flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('trashed') === 'only' ? 'bg-background text-destructive shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-destructive hover:bg-background/50' }}">
                Trash
    ...
    <div id="users-table-container" x-data="{ selected: [] }">
        <!-- Toolbar Section -->
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
            ...
            <!-- Right: Add Action -->
            <a href="/users/create" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary/90 hover:scale-[1.02] active:scale-95 transition-all duration-200">
        ...
        <div class="rounded-2xl border border-border/40 bg-card/50 backdrop-blur-xl shadow-sm overflow-hidden relative">
            ...
            <div class="relative w-full overflow-auto">
                <table class="w-full caption-bottom text-sm">
                        ...
                        <tr class="border-b border-border/40 transition-colors hover:bg-muted/30 data-[state=selected]:bg-muted bg-muted/20">
                            ...
                            <th class="h-12 w-[50px] px-6 text-left align-middle">
                                <div class="flex items-center">
                                    ...
                                    @click="selected = $event.target.checked ? [{{ $users->pluck('id')->join(',') }}] : []">
                                </div>
                            </th>
                            <th class="h-12 px-6 text-left align-middle font-medium text-muted-foreground/70 uppercase tracking-wider text-[11px]">User</th>
                            <th class="h-12 px-6 text-left align-middle font-medium text-muted-foreground/70 uppercase tracking-wider text-[11px]">Role</th>
                    ...
                    <tbody class="[&_tr:last-child]:border-0 text-sm">
                        ...
                        <tr class="group border-b border-border/40 transition-all duration-200 hover:bg-muted/40 data-[state=selected]:bg-muted/60 {{ $user->trashed() ? 'bg-destructive/5' : '' }}">
                            <!-- Row Checkbox -->
                            ...
                            <td class="p-6 align-middle">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex flex-col space-y-0.5">
                                        <span class="font-semibold text-foreground text-sm tracking-tight">{{ $user->name }}</span>
                            ...
                            <td class="p-6 align-middle">
                                @foreach($user->roles as $role)
                                    <span class="inline-flex items-center rounded-md border border-border bg-secondary/50 px-2.5 py-0.5 text-xs font-medium text-secondary-foreground transition-colors hover:bg-secondary">
                                        {{ $role->name }}
                                    </span>
                                @endforeach
                            </td>
                                 ...
                                 <div class="relative flex justify-end" x-data="{ open: false }" @click.away="open = false">
                                    ...
                                    <div x-show="open" 
                                        ...
                                        @if($user->trashed())
                                            ...
                                            <form action="/users/{{ $user->id }}" method="POST" onsubmit="return confirm('Permanently delete?');">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <!-- RELATIVE URL -->
                                            <a href="/users/{{ $user->id }}/edit" class="flex w-full cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground">
                                            ...
                                            <form action="/users/{{ $user->id }}" method="POST" onsubmit="return confirm('Move to trash?');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="p-16 text-center">
                                <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                    ...
                                    <p class="text-sm mt-1 max-w-xs mx-auto">Try adjusting your safe terms or add a new user to get started.</p>
                                    @if(request('search'))
            ...
            <div class="border-t border-border/40 p-4 bg-muted/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                 <div class="text-xs text-muted-foreground px-2">Page <span class="font-medium text-foreground">{{ $users->currentPage() }}</span> of <span class="font-medium">{{ $users->lastPage() }}</span></div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadContent(url, pushState = true) {
            ...
            try {
                ...
                if (newContent) {
                    ...
                    // Re-initialize Alpine.js for the new DOM elements
                    // This ensures dropdowns and other interactive elements work after AJAX swap
                    if (typeof Alpine !== 'undefined') {
                        Alpine.initTree(container);
                    }
                } else {
                    window.location.href = url; // Fallback if ID mismatch
                }
            } catch (err) {
        ...
        // --- Event 1: Browser Back/Forward Handling ---
        window.addEventListener('popstate', () => {
            loadContent(window.location.href, false);
        });

        // --- Event 2: Pagination Clicks ---
        ...
        // --- Event 3: Search Input Debounce (Type to Search) ---
        ...
        // --- Event 4: Dropdown/Filters Change ---
        ...
        // --- Event 5: Prevent Default Form Submit for AJAX Forms ---
        container.addEventListener('submit', (e) => {
            if (e.target.id === 'search-form' || e.target.id === 'per-page-form') {
                e.preventDefault();
                // Logic already handled by input/change listeners, but this catches "Enter" key
                const form = e.target;
                const url = new URL(form.action);
                const params = new URLSearchParams(new FormData(form));
                loadContent(`${url.origin}${url.pathname}?${params.toString()}`);
