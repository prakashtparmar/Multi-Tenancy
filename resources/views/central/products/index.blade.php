...
<div id="products-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div class="space-y-1">
            <h1 class="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Products</h1>
        ...
        <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
            <a href="{{ route('central.products.index') }}" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === null ? 'bg-background text-foreground shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-foreground hover:bg-background/50' }}">
                All Products
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="{{ route('central.products.index', ['status' => 'active']) }}" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('status') === 'active' ? 'bg-background text-emerald-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-emerald-600 hover:bg-background/50' }}">
                Active
            </a>
            <div class="w-px h-4 bg-border/40 mx-1"></div>
            <a href="{{ route('central.products.index', ['stock' => 'low']) }}" class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request('stock') === 'low' ? 'bg-background text-amber-600 shadow-sm ring-1 ring-border/20' : 'text-muted-foreground hover:text-amber-600 hover:bg-background/50' }}">
                Low Stock
            </a>
        </div>
    </div>
    ...
    <div id="products-table-container" x-data="{ selected: [] }">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
            ...
            <a href="{{ route('central.products.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary/90 hover:scale-[1.02] active:scale-95 transition-all duration-200">
                    ...
                    <tbody class="[&_tr:last-child]:border-0 text-sm">
                        ...
                        <tr class="group border-b border-border/40 transition-all duration-200 hover:bg-muted/40 data-[state=selected]:bg-muted/60">
                             <td class="p-6 align-middle">
                                <div class="flex items-center">
                                    <input type="checkbox" value="{{ $product->id }}" x-model="selected" class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary">
                                </div>
                            </td>
                            <td class="p-6 align-middle">
                                <div class="flex flex-col space-y-0.5">
                                    <span class="font-semibold text-foreground text-sm tracking-tight">{{ $product->name }}</span>
                                    <span class="text-xs text-muted-foreground font-mono">{{ $product->sku }}</span>
                                </div>
                            </td>
                            <td class="p-6 align-middle">
                                @if($product->category)
                            ...
                            <td class="p-6 align-middle">
                                <span class="capitalize text-xs font-medium">{{ $product->type }}</span>
                                @if($product->is_organic)
                                    <span class="ml-2 inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">Organic</span>
                                @endif
                            </td>
                            <td class="p-6 align-middle">
                            ...
                            <td class="p-6 align-middle">
                                ...
                                @elseif($product->stock_on_hand > 0)
                                    <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold bg-amber-500/10 text-amber-600 border border-amber-500/20">
                                        {{ $product->stock_on_hand }} low stock
                                    </span>
                                @else
                            ...
                            <td class="p-6 align-middle text-right">
                                 <div class="relative flex justify-end" x-data="{ open: false }" @click.away="open = false">
                                    ...
                                    <div x-show="open" class="absolute right-0 top-9 z-50 min-w-[180px] overflow-hidden rounded-xl border border-border/60 bg-popover/95 p-1 text-popover-foreground shadow-xl shadow-black/5 backdrop-blur-xl" style="display: none;">
                                        ...
                                        <a href="{{ route('central.products.edit', $product) }}" class="flex w-full cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground">
                                            Edit Details
                                        </a>
                                        <div class="my-1 h-px bg-border/50"></div>
                                        <form action="{{ route('central.products.destroy', $product) }}" method="POST" onsubmit="return confirm('Delete product?');">
                                            @csrf
                                            @method('DELETE')
