...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm shadow-sm overflow-hidden p-6">
        <form action="{{ route('central.categories.update', $category) }}" method="POST" class="space-y-6" x-data="{ name: '{{ addslashes($category->name) }}', slug: '{{ $category->slug }}' }">
            @csrf
            @method('PUT')
            ...
            <!-- Parent Category -->
            <div class="space-y-2">
                <label class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Parent Category</label>
            ...
            <div class="flex items-center space-x-2">
                ...
                <input type="checkbox" name="is_active" value="1" id="is_active" class="h-4 w-4 rounded border-primary text-primary shadow focus:ring-primary" {{ $category->is_active ? 'checked' : '' }}>
