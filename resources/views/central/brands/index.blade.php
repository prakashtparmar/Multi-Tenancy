...
@section('content')
    <div id="brands-page-wrapper" class="flex flex-1 flex-col space-y-8 p-8 animate-in fade-in duration-500">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                <h1
                    ...
                    Brands</h1>
                <p class="text-muted-foreground text-sm">Manage product brands and manufacturers.</p>
            </div>
            <div class="flex items-center p-1 bg-muted/50 rounded-xl border border-border/50 backdrop-blur-sm">
                <a href="{{ route('central.brands.index') }}"
                ...
                <a href="{{ route('central.brands.index', ['status' => 'active']) }}"
        ...
        <div id="brands-table-container" x-data="{ selected: [] }">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl">
                ...
                <a href="{{ route('central.brands.create') }}"
                                ...
                                <th class="h-12 w-[50px] px-6 text-left align-middle">
                                    <div class="flex items-center">
                                        <input type="checkbox"
                                            ...
                                            @click="selected = $event.target.checked ? [{{ $brands->pluck('id')->join(',') }}] : []">
                                    </div>
                                </th>
                                <th
                            ...
                            @forelse($brands as $brand)
                                <tr
                                    ...
                                    <td class="p-6 align-middle">
                                        <div class="flex items-center">
                                            <input type="checkbox" value="{{ $brand->id }}" x-model="selected"
                                                class="h-4 w-4 rounded border-input text-primary focus:ring-primary/20 bg-background cursor-pointer transition-all checked:bg-primary checked:border-primary">
                                        </div>
                                    </td>
                                    <td class="p-6 align-middle">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex flex-col space-y-0.5">
                                                ...
                                                <span class="text-xs text-muted-foreground font-mono">{{ $brand->slug }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-6 align-middle">
                                        @if($brand->is_active)
                                            <span
                                                class="inline-flex items-center rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-600">
                                                Active
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center rounded-md border border-muted bg-muted/50 px-2.5 py-0.5 text-xs font-medium text-muted-foreground">
                                                Inactive
                                            </span>
                                        @endif
                                    </td>
                                        ...
                                        <div class="relative flex justify-end" x-data="{ open: false }"
                                            ...
                                            <div x-show="open"
                                                ...
                                                <div
                                                    ...
                                                    Manage</div>
                                                <a href="{{ route('central.brands.edit', $brand) }}"
                                                ...
                                                <form action="{{ route('central.brands.destroy', $brand) }}" method="POST"
                                                    onsubmit="return confirm('Delete brand?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                ...
                                <tr>
                                    <td colspan="4" class="p-16 text-center">
                                        <div class="flex flex-col items-center justify-center text-muted-foreground/50">
                                            <p class="text-lg font-semibold text-foreground">No brands found</p>
                                            <p class="text-sm mt-1">Add your first brand.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if($brands->hasPages())
                    <div
                        ...
                        <div class="text-xs text-muted-foreground px-2">Page <span
                                class="font-medium text-foreground">{{ $brands->currentPage() }}</span> of <span
                                class="font-medium">{{ $brands->lastPage() }}</span></div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadContent(url, pushState = true) {
                ...
                try {
                    ...
                    const newContent = doc.getElementById('brands-table-container');
                    if (newContent) {
                        container.innerHTML = newContent.innerHTML;
                        if (pushState) window.history.pushState({}, '', url);
                        // Re-init Alpine if needed
                    } else {
