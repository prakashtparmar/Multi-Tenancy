<div x-data="chatWidget()" x-init="init()" x-cloak class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
    ...
    <div x-show="isOpen" 
        ...
        <div class="bg-primary text-primary-foreground p-4 flex justify-between items-center shadow-sm">
            <div class="flex items-center gap-2">
                <template x-if="currentView === 'chat'">
                    <button @click="setView('list')" class="hover:bg-primary/20 p-1 rounded-full transition-colors mr-1">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-left"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
                    </button>
                </template>
                <template x-if="currentView === 'create_group'">
                    <button @click="setView('list')" class="hover:bg-primary/20 p-1 rounded-full transition-colors mr-1">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-left"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
                    </button>
                </template>
        ...
        <div x-show="currentView === 'list'" class="flex-1 overflow-y-auto p-2 space-y-1 relative">
            <div class="p-2 sticky top-0 bg-white dark:bg-zinc-900 z-10">
                <input type="text" x-model="searchQuery" @input.debounce.300ms="fetchUsers()" placeholder="Search users..." class="w-full px-3 py-2 text-sm rounded-md border border-input bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring">
            </div>
            
            <div class="px-2 pb-2">
                <button @click="setView('create_group')" class="w-full py-2 px-3 bg-primary/10 text-primary hover:bg-primary/20 rounded-md text-sm font-medium transition-colors flex items-center justify-center gap-2">
            ...
            <template x-for="user in users" :key="user.group_id != '0' ? 'g' + user.id : 'u' + user.id">
                <button @click="selectUser(user)" class="w-full flex items-center gap-3 p-3 hover:bg-muted/50 rounded-lg transition-colors text-left group">
                    <div class="relative">
                        <template x-if="user.photo">
                             <img :src="user.photo" class="w-10 h-10 rounded-full object-cover border border-border">
                        </template>
                        <template x-if="!user.photo">
                            <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold border border-border">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-baseline mb-0.5">
                            <p class="font-medium text-sm truncate text-foreground group-hover:text-primary transition-colors" x-text="user.name"></p>
                            <span class="text-[10px] text-muted-foreground" x-text="user.last_message_time || ''"></span>
                        </div>
                        <p class="text-xs text-muted-foreground truncate" x-text="user.last_message || 'No messages yet'"></p>
                    </div>
                    <template x-if="user.unread_msg > 0">
                        <span class="bg-primary text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center" x-text="user.unread_msg"></span>
                    </template>
                </button>
            </template>
        </div>
        ...
        <!-- Create Group View -->
        <div x-show="currentView === 'create_group'" class="flex-1 flex flex-col h-full bg-background p-4 overflow-y-auto">
            <div class="space-y-4">
                <div>
                    <label class="text-sm font-medium mb-1 block">Group Name</label>
                    <input type="text" x-model="newGroupName" placeholder="Enter group name..." class="w-full px-3 py-2 text-sm rounded-md border border-input bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring">
                </div>
                
                <div class="flex-1">
                    <label class="text-sm font-medium mb-1 block">Select Members</label>
                    <input type="text" x-model="userSearchQuery" @input.debounce.300ms="searchUsersForGroup()" placeholder="Search users..." class="w-full px-3 py-2 text-sm rounded-md border border-input bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring mb-2">
                    ...
                    <div class="max-h-48 overflow-y-auto border rounded-md p-2 space-y-1">
                        <template x-for="user in availableUsers" :key="user.id">
                            <label class="flex items-center gap-2 p-2 hover:bg-muted/50 rounded cursor-pointer">
                                <input type="checkbox" :value="user.id" x-model="selectedGroupMembers" class="rounded border-gray-300 text-primary focus:ring-primary">
                ...
                <button @click="createGroup()" :disabled="!newGroupName || selectedGroupMembers.length === 0" class="w-full py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                    Create Group
                </button>
            </div>
        </div>
        ...
        <div x-show="currentView === 'chat'" class="flex-1 flex flex-col h-full overflow-hidden">
                    ...
                    <div class="flex flex-col" :class="msg.send_by_you ? 'items-end' : 'items-start'">
                        <div class="max-w-[85%] rounded-2xl px-4 py-2 text-sm shadow-sm relative group"
                            ...
                            <template x-if="msg.parent_message_id">
                                <div class="mb-2 p-2 rounded bg-black/10 dark:bg-white/10 text-xs border-l-2 border-primary/50">
                                   <div class="opacity-70 truncate" x-html="msg.parent_message_body || 'Attachment'"></div>
                                </div>
                            </template>
                            ...
                            <template x-if="msg.is_image">
                                <div class="mb-2 mt-1">
                                    <img :src="msg.s3_url" class="rounded-lg max-h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity" @click="window.open(msg.s3_url, '_blank')">
                                </div>
                            </template>
                            
                            <!-- File Attachment -->
                            <template x-if="msg.attachment && !msg.is_image">
                                 <a :href="msg.s3_url" target="_blank" class="flex items-center gap-2 mb-2 bg-black/10 dark:bg-white/10 p-2 rounded hover:bg-black/20 transition-colors">
                                    ...
                                    <span class="truncate underline decoration-dotted" x-text="msg.type || 'File'"></span>
                                 </a>
                            </template>
            ...
            <div class="p-3 bg-background border-t border-border z-20 relative">
                ...
                <div class="flex items-end gap-2">
                    <button @click="$refs.fileInput.click()" class="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full transition-colors flex-shrink-0">
    ...
    <button @click="toggleChat()" 
        ...
        <span class="absolute -top-1 -right-1 flex h-4 w-4" x-show="totalUnread > 0">
            ...
            <span class="relative inline-flex rounded-full h-4 w-4 bg-red-500 text-[10px] font-bold text-white items-center justify-center" x-text="totalUnread > 9 ? '9+' : totalUnread"></span>
        </span>

        <svg x-show="!isOpen" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-message-circle"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg>
        <svg x-show="isOpen" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down"><path d="m6 9 6 6 6-6"/></svg>
    </button>
</div>
        ...
        return {
            ...
            async searchUsersForGroup() {
                 try {
                    ...
                    if (data.success) {
                        // Filter out existing groups if possible, but get_users returns mixed.
                        // We only want 'user' type items for group creation ideally.
                        // Based on ChatServices, it returns all matching users.
                        // Items have 'recipient_id' if they are users.
